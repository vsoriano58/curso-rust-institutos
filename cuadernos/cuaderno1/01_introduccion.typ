...
- *¿Qué es un Repositorio?*: Es el lugar donde se guardan todos los archivos de un proyecto junto con todo el historial de su "máquina del tiempo". Es decir, el repositorio guarda para cada archivo, todas las versiones por las que ha pasado a través del tiempo hasta su estado actual.
...
1. Abre la Terminal (búscala en el `Launchpad` o con `Spotlight` presionando `Cmd + Espacio`).
...
git --version
```
3. Si no lo tienes instalado, aparecerá una ventana emergente de Apple preguntándote si deseas instalar las *Herramientas de línea de comandos de Xcode*. Haz clic en Instalar y acepta las condiciones. ¡El sistema hará todo el trabajo por ti!
...
+ Al finalizar, tendrás un nuevo programa instalado llamado *Git Bash*. Es una terminal especial para Windows desde la cual podrás usar todos los comandos de Git exactamente igual que si estuvieras en Linux o Mac.
...
Para comprobar que *Git* se ha instalado bien, escribe el comando `git --version`. Si sale un número de versión, ¡ya lo tienes!
