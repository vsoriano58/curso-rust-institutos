...
Hasta este punto de tu aventura con Rust, has aprendido a manejar la información utilizando variables individuales. Si necesitabas almacenar la vida de un jugador, creabas un *let salud = 100;*. Si querías su nombre, añadías un *let nombre = String::from(`"`Link`"`);*.
...
Imagina que estás diseñando el código de un videojuego multijugador. Cada personaje en el mapa necesita un nombre, una posición en el eje X, una posición en el eje Y y un estado de energía. Usando únicamente lo aprendido en los cuadernos anteriores, tu código inicial se vería más o menos así:
...
fn main() {
    ...
    // Si tuviéramos 50 jugadores, ¡necesitaríamos 200 variables sueltas!
}
```
Para nuestra mente humana, j1_nombre y j1_energia están íntimamente relacionados: pertenecen al mismo personaje. Pero para el compilador de Rust, estas variables son completas desconocidas. Son solo datos flotando de forma independiente en la memoria de la computadora.
...
+ *Falta de cohesión:* Las funciones que modifican al jugador (como recibir daño o teletransportarse) requieren que les pases una lista kilométrica de argumentos individuales en los parámetros de la función.

+ *Escalabilidad imposible:* Crear dinámicamente nuevos personajes o eliminarlos cuando se desconecten de la partida se vuelve una tarea titánica si sus propiedades están dispersas.
