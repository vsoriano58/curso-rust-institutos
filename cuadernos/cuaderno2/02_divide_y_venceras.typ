...
Una función se comporta como una máquina de funa ábrica: le introduces una materia prima (*parámetros*), realiza un proceso interno, y te devuelve un producto terminado (valor *de retorno*).
...
let mi_edad = calcular_edad(2026, 2010); // Llamamos a la máquina
```
En esta línea declaramos la variable mi_edad con let (inmutable) y le asignamos el valor de retorno de la función calcular_edad pasandole como parámetro anio_actual el valor 2026 y como parámetro anio_nacimiento el valor 2010.

En este momento, la ejecución del programa pasa a la primera línea de la función, que hace la resta 2026 - 2010 = 16 y vuelve al main() asignando este valor a mi_edad.
...
```
💡 Consejo: Si por error le pones un punto y coma a esa última línea, el compilador se quejará diciendo que la función devuelve "nada" () en lugar de un i32 tal como decimos en la cabecera de la función al definirla. ¡Prueba a quitarlo y verás la magia!
...
```
#nota("La función calcular_factorial utiliza un algorítmo tipico para calcular el factorial de un número mediante una función recursiva, es decir, que se llama a simisma. Lo hemos incluido a modo de ejemplo de una función pero no debe preocuparte si no entiendes su código. Prueba simplemente calcular_factorial(4) que debe darte 4x3x2x1 = 24")
...
// 2. Traemos las funciones al entorno actual para usarlas cómodamente
use operaciones::{calcular_potencia, calcular_factorial};

fn main() {
    println!("=== CALCULADORA CIENTÍFICA MODULAR ===");

    // Ejemplo 1: Probando la potencia (f64 elevado a i32)
    let base = 2.5;
    let exp = 3;
    let potencia = calcular_potencia(base, exp);
    println!("La potencia de {} elevado a {} es: {}", base, exp, potencia);

    // Ejemplo 2: Probando el factorial
    let numero = 5;
...
- *El Sistema de Módulos (mod y pub)*: Por defecto, todo en Rust es privado (está oculto). Si en operaciones.rs hubiéramos escrito fn calcular_factorial sin nada delante, main.rs no podría ver la función y el compilador daría un error. Al añadir pub, le damos permiso a otros archivos para usarla.
